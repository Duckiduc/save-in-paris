...
import { Card, Typography, Row, Col, Divider, List, Tag, Alert, Statistic } from 'antd';
import { 
  HomeOutlined, 
  CarOutlined, 
  ShoppingOutlined, 
...
const GuideInfo = ({ section = 'overview', userProfile = null }) => {
  ...
  const guideData = {
    ...
    housing: {
      affordableArrondissements: [
        { district: '10e arrondissement', price: 'dès 390€', highlight: 'studios abordables' },
        { district: '19e arrondissement', price: 'excellent rapport qualité/prix', highlight: 'résidentiel' },
        { district: '20e arrondissement', price: 'quartiers en développement', highlight: 'potentiel' },
        { district: '12e arrondissement', price: 'résidentiel et accessible', highlight: 'calme' }
      ],
      suburbs: [
        { name: 'Montreuil', savings: '-40%', description: 'par rapport à Paris intra-muros' },
        { name: 'Saint-Denis', savings: 'très abordable', description: 'logements économiques' },
        { name: 'Pantin', savings: 'créatif', description: 'quartiers en expansion' },
        { name: 'Aubervilliers', savings: 'excellente connexion', description: 'transport optimisé' }
      ]
    },
    transport: {
      navigoAll: 88.80,
      ...
      alternatives: [
        { option: 'Vélo/trottinette', cost: '30-50€/mois', benefit: 'amortissement rapide' },
        ...
        <Card title="📊 Règle Budgétaire 50/30/20" size="small">
          <Row gutter={16}>
            <Col span={8}>
              ...
              />
              <Text type="secondary" style={{ fontSize: '11px' }}>logement, transport, alimentation</Text>
            </Col>
            <Col span={8}>
              <Statistic 
                title="Loisirs" 
              ...
              />
              <Text type="secondary" style={{ fontSize: '11px' }}>sorties, shopping, hobbies</Text>
            </Col>
            <Col span={8}>
              <Statistic 
                title="Épargne" 
              ...
              />
              <Text type="secondary" style={{ fontSize: '11px' }}>investissements, urgence</Text>
            </Col>
          </Row>
        </Card>
      </Col>
      ...
      <Col xs={24} md={12}>
        <Card title="💰 Salaires Moyens Paris" size="small">
          <List size="small" split={false}>
            <List.Item>
              <Text strong>Cadres:</Text> <Text style={{ color: '#78dbff' }}>{guideData.salaries.cadres}€ nets/mois</Text>
            </List.Item>
            <List.Item>
              <Text strong>Intermédiaires:</Text> <Text style={{ color: '#78dbff' }}>{guideData.salaries.intermediaires}€ nets/mois</Text>
            </List.Item>
            <List.Item>
              <Text strong>Employés:</Text> <Text style={{ color: '#78dbff' }}>{guideData.salaries.employes}€ nets/mois</Text>
            </List.Item>
            <List.Item>
              <Text strong>Moyenne:</Text> <Text style={{ color: '#78dbff', fontWeight: 'bold' }}>{guideData.salaries.moyenne}€ nets/mois</Text>
            </List.Item>
          </List>
        </Card>
      </Col>
        ...
        <Card title="🏠 Arrondissements Abordables" size="small">
          <List size="small">
            {guideData.housing.affordableArrondissements.map((area, index) => (
              <List.Item key={index}>
                <List.Item.Meta
                  title={<Text style={{ color: '#78dbff' }}>{area.district}</Text>}
                  description={
                    <>
        ...
        <Card title="🚇 Alternatives Banlieue" size="small">
          <List size="small">
            {guideData.housing.suburbs.map((suburb, index) => (
              <List.Item key={index}>
                <List.Item.Meta
                  title={<Text style={{ color: '#78dbff' }}>{suburb.name}</Text>}
                  description={
                    <>
  ...
  const renderTransportGuide = () => (
    <Card title="🚇 Optimisation Transport" size="small">
      <Row gutter={[16, 16]}>
        <Col xs={24} sm={12}>
          <Paragraph>
            <Text strong>Pass Navigo toutes zones:</Text> <Text style={{ color: '#78dbff' }}>{guideData.transport.navigoAll}€/mois</Text>
          </Paragraph>
          <Paragraph>
            <Text strong>Pass Navigo zones 1-3:</Text> <Text style={{ color: '#78dbff' }}>{guideData.transport.navigo13}€/mois</Text>
          </Paragraph>
          <Alert 
        ...
        <Col xs={24} sm={12}>
          <Text strong style={{ color: '#78dbff' }}>Alternatives économiques:</Text>
          <List size="small">
  ...
  const renderFoodGuide = () => (
    <Card title="🛒 Budget Alimentation Optimisé" size="small">
      <Row gutter={[16, 16]}>
        <Col xs={24} sm={12}>
          <Title level={5} style={{ color: '#78dbff', margin: '8px 0' }}>Budgets par profil:</Title>
          <List size="small">
            <List.Item>
              <Text>Personne seule: </Text>
              <Tag color="orange">{guideData.food.budgets.single.normal}€</Tag>
              <Text> → </Text>
              <Tag color="green">{guideData.food.budgets.single.optimized}€</Tag>
            </List.Item>
            <List.Item>
              <Text>Couple: </Text>
              <Tag color="orange">{guideData.food.budgets.couple.normal}€</Tag>
              <Text> → </Text>
              <Tag color="green">{guideData.food.budgets.couple.optimized}€</Tag>
            </List.Item>
            <List.Item>
              <Text>Famille de 4: </Text>
              <Tag color="orange">{guideData.food.budgets.family4.normal}€</Tag>
              <Text> → </Text>
              <Tag color="green">{guideData.food.budgets.family4.optimized}€</Tag>
            </List.Item>
          </List>
        </Col>
        <Col xs={24} sm={12}>
          <Title level={5} style={{ color: '#78dbff', margin: '8px 0' }}>Stratégies d'économie:</Title>
          <List size="small">
                ...
                <List.Item.Meta
                  title={
                    <span>
                      ...
                      <Tag color="blue" style={{ marginLeft: 8 }}>{product.rate}</Tag>
                    </span>
                  }
  ...
  switch (section) {
    ...
    case 'complete':
      return (
        <div>
          <Title level={3} style={{ color: '#78dbff', textAlign: 'center', marginBottom: 24 }}>
