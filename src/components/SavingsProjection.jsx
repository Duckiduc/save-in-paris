...
const SavingsProjection = ({ monthlySavings, userAge = 30 }) => {
  ...
  const conservative = calculateSavingsProjection(
    monthlySavings,
    projectionYears,
    conservativeReturn
  );
  const moderate = calculateSavingsProjection(
    monthlySavings,
    projectionYears,
    moderateReturn
  );
  const aggressive = calculateSavingsProjection(
    monthlySavings,
    projectionYears,
    aggressiveReturn
  );
  ...
  for (let year = 1; year <= projectionYears; year++) {
    const consProj = calculateSavingsProjection(
      monthlySavings,
      year,
      conservativeReturn
    );
    const modProj = calculateSavingsProjection(
      monthlySavings,
      year,
      moderateReturn
    );
    const aggProj = calculateSavingsProjection(
      monthlySavings,
      year,
      aggressiveReturn
    );
  ...
  return (
    ...
    >
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData}>
            ...
            <Tooltip
              ...
              labelFormatter={(year) =>
                `À ${userAge + year} ans (${year} ans d'épargne)`
              }
            />
            <Line
              type="monotone"
      ...
      <Alert
        ...
        description={`Avec ${formatCurrency(
          monthlySavings
        )} épargnés chaque mois, vous pourriez constituer entre ${formatCurrency(
          conservative.totalWithInterest
        )} et ${formatCurrency(
          aggressive.totalWithInterest
